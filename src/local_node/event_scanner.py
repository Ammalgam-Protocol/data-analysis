...
Modifed from https://web3py.readthedocs.io/en/v5/examples.html#example-code
to scan for swaps and syncs from uniswap v2 pair.
"""
...
class JSONifiedState(EventScannerState):
    ...
    def restore(self):
        ...
        try:
            self.state = json.load(open(self.path, "rt"))
    ...
    def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
        ...
        if event["event"] == 'Sync':
            sync = {"reserve0": args["reserve0"], "reserve1": args["reserve1"]}
            # Record ERC-20 transfer in our database
            self.state["blocks"][block_number][txhash][log_index] = sync
            ...
            self.state["blocks"][block_number][txhash][log_index] = swap
